...
export function InvestorHome({ wallet, onOpenDeal }) {
  ...
  return <main className="market-page pb-20">
    ...
    <div className="page-shell">
      ...
      {loading ? <div className="market-empty">Loading the verified market…</div> : visible.length === 0 ? <div className="market-empty">No approved companies match this view. Start the local seed to populate the test market.</div> : <div className="market-list">{visible.map((business)=><MarketRow key={business.address} business={business} watched={watchlist.includes(business.address)} onWatch={()=>toggleWatch(business.address)} onOpen={()=>business.deal_id != null && onOpenDeal(business.deal_id)}/>)}</div>}
      </section>
